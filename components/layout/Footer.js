...
export default function Footer() {
  const companyLinks = ['About Us', 'Blogs', 'Team', 'Contact Us'];
  const serviceLinks = ['Web Development', 'AI Integration', 'WhatsApp Bots', 'Voice Agents'];

  // START: State for the subscribe form
  const [email, setEmail] = useState('');
  const [error, setError] = useState('');
  const [success, setSuccess] = useState(false);
  // END: State for the subscribe form

  /**
   * Handles the subscription form submission.
   * Validates the email and sets error or success states.
   */
  const handleSubscribe = () => {
    ...
    setError('');
    setSuccess(true);
    // You can add your API call for subscription here
    // For demonstration, we'll just show the success message.
  };

  return (
   <footer className="bg-gradient-to-r from-white via-blue-200 to-blue-100 dark:bg-slate-900 text-gray-600 dark:text-gray-400 border-t border-gray-200 dark:border-gray-800">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {companyLinks.map(link => (
                <li key={link}>
                  <a href={`#${link.toLowerCase().replace(/\s+/g, '-')}`} className="text-gray-500 hover:text-blue-600 transition-colors">
                    {link}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              {serviceLinks.map(link => (
                <li key={link}>
                  <a href="#services" className="text-gray-500 hover:text-blue-600 transition-colors">
                    {link}
                  </a>
                </li>
              ))}
          ...
          {/* Column 4: Achievements & Subscribe */}
          {/* <div/> */}
            ...
            <div className="flex flex-col space-y-4">
              ...
              {/* <div className="flex items-center text-sm text-gray-500">
                ...
                <span>Best Workplace Award</span>
              </div>
            {/* </div> */}

            {/* START: Subscribe Form with Validation */}
            <div className="mt-1">
              ...
              {/* Validation Messages */}
              {error && <p className="text-red-500 text-xs mt-1 ml-1">Invalid Email</p>}
              {success && (
                <div className="flex items-center text-green-600 text-xs mt-1 ml-1">
                  <CheckCircle2 className="w-4 h-4 mr-1" />
                  <span>Subscribed</span>
                </div>
              )}
            </div>
            {/* END: Subscribe Form with Validation */}
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-gray-200">
        <div className="container mx-auto px-4 py-6 flex flex-col sm:flex-row justify-between items-center">
          <p className="text-gray-500 text-sm mb-4 sm:mb-0">&copy; {new Date().getFullYear()} NamoosX. All Rights Reserved.</p>
